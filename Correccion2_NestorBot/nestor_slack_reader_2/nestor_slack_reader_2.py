...
def command():
    if request.method=='POST':
        print("hola, llego el mensaje")
        data=request.form.get("text")
        print(data)
        if data.startswith("[traducir]"):
            channel.basic_publish(exchange='nestor', routing_key="traducir", body=data)
        if data.startswith("[wikipedia]"):
            channel.basic_publish(exchange='nestor', routing_key="wikipedia", body=data)
        return "tu mensaje fue recibido"
...
def message(payload):
    ...
    text = event.get("text")
    print("llego un mensaje {0}".format(text))    
